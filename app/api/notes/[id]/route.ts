...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const { data, error: dbErr } = await supabase.from("notes").select("*").eq("id", params.id).single();
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const { data, error: dbErr } = await supabase.from("notes").update(parsed.data).eq("id", params.id).select().single();
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  const { user, supabase, error: authErr } = await requireUser();
  if (!user) return error(authErr, 401);
  const { error: dbErr } = await supabase.from("notes").delete().eq("id", params.id);
